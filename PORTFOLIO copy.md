...
### Backend (CSBE)
- **Language**: Java 25
- **Framework**: Quarkus 3.36.1 (REST, Jackson)
- **Build Tool**: Gradle
- **Database**: PostgreSQL (Panache / Hibernate ORM)
- **gRPC**: Quarkus gRPC Integration
- **Security**: Quarkus SmallRye JWT, nimbus-jose-jwt (for Guest Token verification)
- **Architecture**: Hexagonal Architecture (Ports and Adapters)
- **External Integration**: Open Data Portal API (Charging Station Info)

### Frontend (CSFE)
- **Framework**: Next.js 16.1.1 (App Router)
- **Library**: React 19.2.3, TypeScript
...
- **Hexagonal Architecture (Backend)**:
  ...
  - **Inbound Adapters**: Web controllers (`adapter.in.web`) handling HTTP requests, and gRPC services (`adapter.in.grpc`) handling RPC calls.
...
- **헥사고날 아키텍처 (백엔드)**:
  ...
  - **인바운드 어댑터**: HTTP 요청을 처리하는 웹 컨트롤러 (`adapter.in.web`) 및 gRPC 서비스를 제공하는 gRPC 핸들러 (`adapter.in.grpc`).
...
- **Deployment**: Vercel (Frontend), Railway (Backend via Docker Container)
